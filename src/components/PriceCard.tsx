...
interface PriceCardProps extends PriceData {
...
const PriceCard: React.FC<PriceCardProps> = ({ source, price, lastUpdate, loading, error, sparklineData, forexRate }) => {
  ...
  return (
    <div className="bg-card-bg border border-card-border rounded-2xl p-6 shadow-card hover:shadow-card-hover transition-shadow duration-300 group hover:scale-[1.02] transform flex flex-col h-full">
      ...
      <div className="mb-4">
        <div className="text-4xl font-bold text-content-primary tabular-nums">
          {error ? 'Error' : price ? price.toFixed(4) : 'N/A'}
        </div>
      ...
      <MiniSparkline 
        data={sparklineData} 
        color={premium === null ? '#6b7280' : premium < 0 ? '#10b981' : '#ef4444'} 
      />
